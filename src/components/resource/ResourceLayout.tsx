import React from 'react';
import {  useNavigate } from 'react-router-dom';
import { ArrowLeft, ChevronRight } from 'lucide-react';
import { COMMON_CONDITIONS_DATA } from '../../data/siteData'; 

interface ResourceLayoutProps {
    children: React.ReactNode;
    currentSlug: string;
    imageSrc?: string;
}

const ResourceLayout: React.FC<ResourceLayoutProps> = ({ children, currentSlug, imageSrc }) => {
    const navigate = useNavigate();
    const otherResources = COMMON_CONDITIONS_DATA.filter((r) => r.slug !== currentSlug).slice(0, 6);

    return (
        <main className="min-h-screen pt-24 sm:pt-32 pb-16 sm:pb-24 bg-gradient-to-b from-[#f3f8ff] via-white to-[#eef6ff] relative overflow-hidden font-sans">
            <div className="absolute top-20 left-10 w-48 sm:w-72 h-48 sm:h-72 bg-blue-200/30 blur-[80px] sm:blur-[120px] rounded-full pointer-events-none select-none" />
            <div className="absolute right-10 bottom-20 w-64 sm:w-96 h-64 sm:h-96 bg-cyan-200/20 blur-[100px] sm:blur-[140px] rounded-full pointer-events-none select-none" />

            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <button
                    onClick={() => navigate(-1)}
                    className="inline-flex items-center gap-2 px-4 sm:px-5 py-2.5 sm:py-3 rounded-xl bg-white/80 backdrop-blur-md border border-white shadow-md text-[13px] sm:text-[14px] font-semibold text-[#575A5F] hover:text-[#0056b3] transition-all mb-6 sm:mb-12 group cursor-pointer"
                >
                    <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" />
                    <span>Go Back</span>
                </button>

                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 lg:gap-12 items-start">
                    <div className="lg:col-span-7 rounded-[24px] sm:rounded-[32px] bg-white/80 backdrop-blur-xl border border-white shadow-[0_20px_60px_rgba(0,0,0,0.06)] p-5 sm:p-8 md:p-10">
                        {children}
                    </div>

                    <div className="lg:col-span-5 space-y-6 sm:space-y-8 lg:sticky lg:top-28">
                        {imageSrc && (
                            <div className="relative overflow-hidden rounded-[24px] sm:rounded-[32px] shadow-[0_20px_60px_rgba(0,86,179,0.18)]">
                                <img
                                    src={imageSrc}
                                    alt="Consultation"
                                    className="w-full h-[240px] sm:h-[320px] object-cover"
                                />
                                <div className="absolute inset-0 bg-gradient-to-t from-[#001d3d]/60 via-transparent to-transparent" />
                                <div className="absolute bottom-4 sm:bottom-5 left-4 sm:left-5 bg-white/90 backdrop-blur-md px-3 sm:px-4 py-1.5 sm:py-2 rounded-full shadow-lg">
                                    <p className="text-[12px] sm:text-[14px] font-semibold text-[#0b1b36] whitespace-nowrap">
                                        Trusted by 10,000+ Patients
                                    </p>
                                </div>
                            </div>
                        )}

                        <div className="rounded-[24px] sm:rounded-[32px] overflow-hidden border border-blue-100 bg-white/85 backdrop-blur-xl shadow-[0_20px_60px_rgba(0,86,179,0.12)]">
                            <div className="px-5 sm:px-7 py-5 sm:py-6 bg-gradient-to-r from-[#003b7a] via-[#0056b3] to-[#0d8bff]">
                                <span className="text-blue-100 font-bold text-[11px] sm:text-[12px] uppercase tracking-[0.1em] block mb-1.5">
                                    Consultation
                                </span>
                                <h3 className="text-[24px] sm:text-[32px] md:text-[40px] font-semibold text-white leading-tight tracking-tight">
                                    Book Appointment
                                </h3>
                                <p className="text-[14px] sm:text-[15px] text-blue-100 leading-[1.5] mt-2 balance">
                                    Schedule an appointment with our specialist
                                </p>
                            </div>

                            <div className="px-5 sm:px-6 pt-4 flex gap-2 flex-wrap">
                                {['Expert Doctors', 'Secure Booking', 'Fast Response'].map((tag, idx) => (
                                    <span key={idx} className="px-2.5 py-1 rounded-full bg-blue-50 text-[#0056b3] text-[11px] sm:text-[12px] font-semibold whitespace-nowrap">
                                        {tag}
                                    </span>
                                ))}
                            </div>

                            <div className="p-3 sm:p-4 bg-white mt-2">
                                <iframe
                                    src="https://healthyheartsgurgaon.com/wp-json/ssa/v1/embed-inner?integration&type=consultation-phone-call&label&types=1&ssa_locale=en_US&sid=b457aa22b6c63b1c8ecbf79839af7aa3fbca632e&booking_url=https%3A%2F%2Fhealthyheartsgurgaon.com%2Fechocardiography%2F&booking_post_id=471&booking_title=Echocardiography&_wpnonce=ae56ad64f6#/"
                                    title="Book a consultation"
                                    width="100%"
                                    height="705"
                                    loading="eager"
                                    className="w-full rounded-2xl border-0 h-[500px] sm:h-[705px]"
                                    style={{ overflow: 'hidden' }}
                                />
                            </div>
                        </div>

                        <div className="rounded-[24px] sm:rounded-[32px] p-5 sm:p-8 bg-white/85 backdrop-blur-xl border border-white shadow-[0_20px_60px_rgba(0,0,0,0.06)]">
                            <span className="text-[#0056b3] font-bold text-[11px] sm:text-[12px] uppercase tracking-[0.1em] mb-2 block">
                                Knowledge Hub
                            </span>
                            <h3 className="text-[24px] sm:text-[32px] md:text-[40px] font-semibold text-[#0b1b36] mb-4 sm:mb-5 leading-tight tracking-tight">
                                Related Topics
                            </h3>
                            <ul className="space-y-1">
                                {otherResources.map((item) => (
                                    <li key={item.slug}>
                                        <div className="py-3 px-3 sm:px-4 rounded-xl sm:rounded-2xl flex items-center justify-between hover:bg-blue-50 transition-all group gap-4 cursor-default">
                                            <span className="text-[14px] sm:text-[15px] text-[#575A5F] leading-[1.5] font-medium group-hover:text-[#0056b3] transition-colors truncate">
                                                {item.title}
                                            </span>
                                            <ChevronRight
                                                size={16}
                                                className="text-gray-400 shrink-0 group-hover:text-[#0056b3] group-hover:translate-x-1 transition-all"
                                            />
                                        </div>
                                    </li>
                                ))}
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </main>
    );
};

export default ResourceLayout;